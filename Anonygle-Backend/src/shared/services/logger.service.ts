...
export class Logger {
  ...
  logInfo(message: string): void {
    this.winstonLogger.info({ message, timestamp: new Date().toISOString(),source:"HTTP" })
  }

  webSocketError(context: {event: string,clientId: string,ip: string,userAgent?: string,message: string,stack?: string,}): void {
    const timestamp = new Date().toISOString()
    ...
    this.winstonLogger.error(log)
    console.error(`[WebSocket ERROR] ${timestamp} - Event: ${context.event} - Client: ${context.clientId} - ${context.message}`)
  }

  webSocketInfo(context: {event: string,clientId: string,ip: string,userAgent?: string,message: string}): void {
    const log = {
    ...
    console.log(`[WebSocket] ${log.timestamp} - ${context.event} - ${context.message}`)
}
